...
class TestModemScraper:
    ...
    def test_fetch_data_url_ordering(self, mocker):
        ...
        # Verify URLs were tried in correct order
        # Technicolor modems (XB7, TC4400) should be tried first
        calls = [call[0][0] for call in mock_get.call_args_list]
        assert calls[0] == "http://192.168.100.1/network_setup.jst"     # Technicolor XB7, TC4400
        assert calls[1] == "http://192.168.100.1/MotoConnection.asp"    # Motorola MB series
        assert len(calls) == 6  # Total URLs to try
    ...
    def test_fetch_data_stops_on_first_success(self, mocker):
        ...
        # Mock successful response on first URL (tests Technicolor XB7, TC4400)
        mock_get = mocker.patch.object(scraper.session, 'get')
        mock_response = mocker.Mock()
