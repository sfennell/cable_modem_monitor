...
class ModemScraper:
    ...
    def _parse_downstream_channels(self, soup: BeautifulSoup) -> list:
        ...
        try:
            # Look for common table structures in Motorola modems
            # This may need adjustment based on your specific modem model
            tables = soup.find_all("table")

            for table in tables:
                # Look for downstream/receive channel tables
                ...
                if header and ("downstream" in header.text.lower() or "receive" in header.text.lower()):
                    ...
                    for row in rows:
                        ...
                        if len(cols) >= 4:  # Typical: Channel, Freq, Power, SNR, Corrected, Uncorrected
                            try:
                                channel_data = {
                                ...
                                # Some modems have corrected/uncorrected in columns 4 and 5
                                if len(cols) >= 6:
    ...
    def _parse_upstream_channels(self, soup: BeautifulSoup) -> list:
        ...
        try:
            ...
            for table in tables:
                # Look for upstream/transmit channel tables
                header = table.find("th")
                if header and ("upstream" in header.text.lower() or "transmit" in header.text.lower()):
                    ...
                    for row in rows:
                        cols = row.find_all("td")
                        if len(cols) >= 3:  # Typical: Channel, Freq, Power
                            try:
                                channel_data = {
                                    "channel": self._extract_number(cols[0].text),
                                    "frequency": self._extract_number(cols[1].text),
